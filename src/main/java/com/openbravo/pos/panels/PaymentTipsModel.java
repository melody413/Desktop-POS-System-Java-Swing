...
// end     
    ...
    private final static String[] PAYMENTHEADERS = {"label.TableID", "label.CompletedTime", "label.Server", "label.paymenttotal", "label.tip", "label.money"};
...
private AppView m_App;
     ...
     * @return
     */
    public static PaymentTipsModel emptyInstance() {
        
        PaymentTipsModel p = new PaymentTipsModel();
...
// JG 16 May 2013 use diamond inference
     ...
     */
    public static PaymentTipsModel loadInstance(AppView app, String openedTime, String userID) throws BasicException {
...
// JG 9 Nov 12
        ...
        List categorys = new StaticSentence(app.getSession()
            , "SELECT a.NAME, sum(c.UNITS), sum(c.UNITS * (c.PRICE + (c.PRICE * d.RATE))) " +
              ...
              "GROUP BY a.NAME"
            , SerializerWriteString.INSTANCE
            , new SerializerReadClass(PaymentTipsModel.CategorySalesLine.class))
...
// end
        ...
        List l = new StaticSentence(app.getSession(),            
            "select payments.id, closedtickets.ticketname, receipts.datenew, people.name, payments.total, payments.tip, payments.cardnum, payments.notes "
                + "from receipts, tickets, closedtickets, people, payments where receipts.id = tickets.id and closedtickets.ticketid = tickets.ticketid "
                + "and closedtickets.openedby = people.id and payments.receipt = receipts.id and receipts.datenew >= TIMESTAMP(?) and payments.payment not like \"%cash%\""
                + " and people.id = '" + p.userID + "'"
            , SerializerWriteString.INSTANCE
            , new SerializerReadClass(PaymentTipsModel.PaymentsLine.class))
            .list(p.openedTime);
        ...
        List<SalesLine> asales = new StaticSentence(app.getSession(),
                ...
                , SerializerWriteString.INSTANCE
                , new SerializerReadClass(PaymentTipsModel.SalesLine.class))
                .list(app.getActiveCashIndex());
        ...
        List removedLines = new StaticSentence(app.getSession()
            ...
            , SerializerWriteString.INSTANCE
            , new SerializerReadClass(PaymentTipsModel.RemovedProductLines.class)) //new SerializerReadBasic(new Datas[] {Datas.STRING, Datas.DOUBLE}))
...
*/
        ...
        List drawerOpenedLines = new StaticSentence(app.getSession()
            ...
            , SerializerWriteString.INSTANCE
            , new SerializerReadClass(PaymentTipsModel.DrawerOpenedLines.class)) //new SerializerReadBasic(new Datas[] {Datas.STRING, Datas.DOUBLE}))
            .list(startDateFormatted);
        ...
        List products = new StaticSentence(app.getSession()
            ...
            , SerializerWriteString.INSTANCE
            , new SerializerReadClass(PaymentTipsModel.ProductSalesLine.class))
            .list(app.getActiveCashIndex());
    ...
    public AbstractTableModel getPaymentsModel() {
        return new AbstractTableModel() {
            ...
            public Object getValueAt(int row, int column) {
                ...
                switch (column) {
                case 0: return l.getTicketName();
                case 1: return l.getClosedTime();
                case 2: return l.getOpenedBy();
                case 3: return l.getAmount();
                case 4: return l.getTip();
                case 5: return l.getTotal();
                default: return null;
                }
            }
        };
...
// end
    ...
    public static class PaymentsLine implements SerializableRead {
        ...
        @Override
        public void readValues(DataRead dr) throws BasicException {
            paymentID = dr.getString(1);
            ticketname = dr.getString(2);
            closedTime = dr.getString(3);
            opendedBy = dr.getString(4);
            amount = dr.getDouble(5);
            tip = dr.getDouble(6);
            total = round(amount + tip, 2);

            cardNum = dr.getString(7);
            cardExpDate = dr.getString(8);
        }
