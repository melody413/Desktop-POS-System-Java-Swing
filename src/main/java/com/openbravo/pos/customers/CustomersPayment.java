...
//        btnPay.setEnabled(customer.getCurdebt() != null && customer.getCurdebt().doubleValue() > 0.0);
    ...
    public void printTicket(String resname, TicketInfo ticket, CustomerInfoExt customer, Object[] customData) {
        ...
        if (resource == null) {
            ...
            try {
                ...
                if(customData != null) {
                    String paymentType = (String) customData[4];

                    List<Object[]> cashAmount = new ArrayList<>();
                    int cashTotal = 0;
                    for(int i = 0; i < 6; i ++) {
                        if(Double.parseDouble(customData[i + 5].toString()) != 0) {
                            Object[] temp = new Object[3];
                            temp[0] = customData[i + 11];
                            temp[1] = Integer.parseInt(customData[i + 5].toString()) / Integer.parseInt(customData[i + 11].toString());
                            temp[2] = customData[i + 5];
                            cashAmount.add(temp);

                            cashTotal += Integer.parseInt(customData[i + 5].toString());
                        }
                    }

                    if("cashin".equals(paymentType)) {
                        script.put("cashType", "Cash In");
                    } else {
                        script.put("cashType", "Cash Out");
                        try {
                            List<Object[]> lastCashIn = dlsystem.getLastCashIn();
                            double totalOrderCash = dlsystem.getTotalOrderCash();
                            int cashInTotal = 0;
                            for (int i=0; i<lastCashIn.size(); i++) {
                                cashInTotal += Integer.parseInt(lastCashIn.get(i)[2].toString());
                            }

                            double shortage = cashInTotal + totalOrderCash + cashTotal;

                            script.put("cashInAmount", lastCashIn);
                            script.put("cashInTotal", cashInTotal);

                            script.put("totalOrderCash", new BigDecimal(totalOrderCash).setScale(2, RoundingMode.HALF_EVEN));
                            script.put("calculatedTotal", new BigDecimal(cashInTotal + totalOrderCash).setScale(2, RoundingMode.HALF_EVEN));
                            script.put("shortage", new BigDecimal(shortage).setScale(2, RoundingMode.HALF_EVEN));
                        } catch (BasicException e) {}
                    }

                    script.put("cashAmount", cashAmount);
                    script.put("cashTotal", cashTotal);
                }
...
// JG 6 May use multicatch
    ...
    private void btnPayActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPayActionPerformed
        ...
        if (paymentdialog.showDialog(customerext.getAccdebt(), null, 1, 1) > 0) {
    ...
    private void btnPrePayActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnPrePayActionPerformed
        ...
        if (txtPrePay.getText() != null) {
            ...
            if (paymentdialog.showDialog(prepay, null, 1, 1) > 0) {            
