...
public class VirtualKeyboard implements FocusListener {
     ...
     * @param m_comp The component is shown by this.
     */
    public void show(JDialog frame, JPanel keyboardPanel, Component m_comp) {
        this.frame = frame;
...
//        currentComponent = frame.getFocusTraversalPolicy().getFirstComponent(frame);
        ...
        keyboardPanel.add(initRow(row5, new Dimension(keyboardPanel.getWidth(), keyboardPanel.getHeight() / 2)));
        frame.pack();
    }
    ...
    private void actionListener(Key key) {
        ...
        switch (key.keyCode) {
            ...
            case KeyEvent.VK_BACK_QUOTE:
                ...
                backentirespacePressed(currentTextComponent);
                break;
    ...
    private void backentirespacePressed(JTextComponent component) {
        if (currentComponent instanceof JTextComponent) {
            int caretPosition = component.getCaretPosition();
            if (!component.getText().isEmpty() && caretPosition > 0) {
                component.setText("");
